...
public class TekvLSCreateNewConsumptionEvent {
	/**
	 * This function listens at endpoint "/v1.0/consumptionEvent". Two ways to
	...
	public HttpResponseMessage run(
			@HttpTrigger(name = "req", methods = {
					HttpMethod.POST }, authLevel = AuthorizationLevel.ANONYMOUS, route = "consumptionEvent") HttpRequestMessage<Optional<String>> request,
		...
		/**
		 * Get the device matrix id and tokens of the dut + calling platform type
		 * combinations to insert it in the consumption event records
		 */
		// Build SQL statement
	...
	public HttpResponseMessage createLicenseConsumptionEvent(Claims tokenClaims, JSONObject jobj,
		...
		// Build the sql query to get tokens consumption and granularity from device
		// table
		...
		String insertSql = "INSERT INTO license_consumption (subaccount_id, consumption_matrix_id, device_id, calling_platform_id, consumption_date, usage_type, tokens_consumed, modified_by, modified_date, project_id, comment) "
		...
		try (Connection connection = DriverManager.getConnection(dbConnectionUrl);
			...
			insertStmt.setString(11, jobj.has(OPTIONAL_PARAMS.COMMENT.value)
	...
	private boolean createUsageDetail(JSONObject consumptionObj, Connection connection, final ExecutionContext context)
			throws SQLException {
		...
		try (PreparedStatement insertUsageStmt = connection.prepareStatement(sql)) {
			...
			final JSONArray usageDays = consumptionObj.has(OPTIONAL_PARAMS.USAGE_DAYS.value)
					? consumptionObj.getJSONArray(OPTIONAL_PARAMS.USAGE_DAYS.value)
					: null;
			if (usageDays != null && usageDays.length() > 0) {
				...
				if (defaultUsageDay == 7)
					defaultUsageDay = 0;
				insertUsageStmt.setDate(2, Date.valueOf(consumptionDate));
				insertUsageStmt.setInt(3, defaultUsageDay);
				insertUsageStmt.setString(4,
						consumptionObj.has(OPTIONAL_PARAMS.MAC_ADDRESS.value)
								? consumptionObj.getString(OPTIONAL_PARAMS.MAC_ADDRESS.value)
								: null);
				insertUsageStmt.setString(5,
						consumptionObj.has(OPTIONAL_PARAMS.SERIAL_NUMBER.value)
								? consumptionObj.getString(OPTIONAL_PARAMS.SERIAL_NUMBER.value)
								: null);
				insertUsageStmt.addBatch();
			}
			context.getLogger().info("Execute create usages SQL statement (User: " + consumptionObj.getString("userId")
					+ "): " + insertUsageStmt);
