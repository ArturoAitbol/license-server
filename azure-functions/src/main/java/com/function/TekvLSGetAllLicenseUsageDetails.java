...
public class TekvLSGetAllLicenseUsageDetails {
	...
	public HttpResponseMessage run(
		...
		if(!hasPermission(roles, Permission.GET_ALL_LICENSE_USAGE_DETAILS)){
			JSONObject json = new JSONObject();
			...
			switch (view.toLowerCase()) {
				...
				default: {
					...
					for (Condition<String, String> commonCondition: commonConditions) {
						...
						weeklyConfigTokensConsumedQueryBuilder.appendCustomCondition(commonCondition.sql, commonCondition.value);
						tokensConsumedByProjectQueryBuilder.appendCustomCondition(commonCondition.sql, commonCondition.value);
						tokensConsumedQueryBuilder.appendCustomCondition(commonCondition.sql, commonCondition.value);
					}
					...
					weeklyConfigTokensConsumedQueryBuilder.appendOrderBy("consumption_date", SelectQueryBuilder.ORDER_DIRECTION.DESC);

					// Conditions for all tokens per project query
					tokensConsumedByProjectQueryBuilder.appendGroupBy("p.id, p.name, p.status");
					tokensConsumedByProjectQueryBuilder.appendOrderBy("p.name", SelectQueryBuilder.ORDER_DIRECTION.DESC);
