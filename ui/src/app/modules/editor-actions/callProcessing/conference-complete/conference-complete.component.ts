...
@Component({
  selector: 'app-conference-complete',
  templateUrl: './conference-complete.component.html',
  styleUrls: ['./conference-complete.component.css']
})
export class ConferenceCompleteComponent implements OnInit, OnDestroy {
  action: any;
  ...
  ngOnInit() {
    ...
    this.resourcesBk = this.aeService.getFilteredResources(['Phone']).filter(e =>(!e.model || (e.model && e.model.toUpperCase() !== Constants.Webex)));;
  ...
  createAction() {
    ...
    // tslint:disable-next-line: max-line-length
    let query = this.selectedConversation + '.' + this.selectedDevice + '.conferenceComplete(' + this.selectedIntermediateConv;
